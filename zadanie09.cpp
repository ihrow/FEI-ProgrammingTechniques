...
int min(const BinarySearchTree *tree) {
    ...
    Node *node = tree->root;
    while (node->smaller != nullptr) node = node->smaller;
    return node->value;
}
...
unsigned depth(const BinarySearchTree *tree, int value) {
    ...
    Node *node = tree->root;
    size_t depth = 0;
    for (; node != nullptr; depth++) {
        if (node->value == value) return depth;
        if (node->value > value) node = node->smaller;
        else node = node->greater;
    }
...
list<int> path(const BinarySearchTree *tree, int value) noexcept {
    ...
    return path; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
size_t count(const BinarySearchTree *tree) noexcept {
    ...
    for (; !stack.empty(); count++) {
        Node *node = stack.top();
        stack.pop();
        if (node->smaller != nullptr) stack.push(node->smaller);
        if (node->greater != nullptr) stack.push(node->greater);
    }
...
void inorder(Node *node, list<int> &list) {
    if (node) {
        inorder(node->smaller, list);
        list.push_back(node->value);
        inorder(node->greater, list);
    }
}
...
list<int> all(const BinarySearchTree *tree) noexcept {
    list<int> all;
    if (tree->root == nullptr) return all;

    inorder(tree->root, all);

    return all; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
size_t nodesGreaterThanValue(Node *node, int value) {
    if (node == nullptr) {
        return 0;
    }
    if (node->value < value) {
        return nodesGreaterThanValue(node->greater, value);
    }
    else {
        return nodesGreaterThanValue(node->smaller, value) + nodesGreaterThanValue(node->greater, value) + 1;
    }
}
...
size_t countGreater(const BinarySearchTree *tree, int value) noexcept {
    ...
    size_t smaller = nodesGreaterThanValue(tree->root->smaller, value);
    size_t greater = nodesGreaterThanValue(tree->root->greater, value);
    return (tree->root->value > value ? 1 : 0) + smaller + greater;
}
...
void deleteNodes(Node *node) {
    if (node) {
        deleteNodes(node->smaller);
        deleteNodes(node->greater);
        delete node;
    }
}
...
void binarySearch(const vector<int> &data, int value, unsigned &count) {
    ...
    if (data[middle] == value) {
        ...
        binarySearch(left, value, count);
    }
    else {
        count++;
        vector<int> right(data.begin() + middle + 1, data.end());
        binarySearch(right, value, count);
    }
}
...
map<string, size_t> histogram(const vector<string> & data) noexcept {
    ...
    for (const string &word : data) histogram[word]++;
...
map<string, set<size_t>> index(const vector<string> & data) noexcept {
    ...
    for (size_t i = 0; i < data.size(); i++) index[data[i]].insert(i);
    return index; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
