...
ostream &operator<<(ostream &os, const Complex &c) {
    os << c.getRealPart() << (c.getImaginaryPart() < 0 ? "" : "+") << c.getImaginaryPart() << "i";
    return os;
}
...
list<char> depthFirstSearchUpperCases(const Tree *tree) {
    ...
    s.push(tree->root);
    while (!s.empty()) {
        if (isupper(s.top()->value)) {
            result.push_back(s.top()->value);
        }
        Node *node = s.top();
        s.pop();
        for (Node* checked : q){
            if (node->value == checked->value) {
                b = false;
            }
        }
        if (b) {
...
list<string> breadthFirstSearchReachable(Planet * planet, const string & startCity) {
    ...
    while (!q.empty()) {
        ...
        for (RoadTo &r : c->roads) {
            if (!r.city->searchData.discovered) {
                r.city->searchData.discovered = true;
                q.push(r.city);
            }
        }
    }
...
map<string, unsigned> dijkstra(Planet * planet, const string & startCity) {
    ...
    result[start->name] = start->searchData.distance;
    ...
    while (!q.empty()) {
        current = q.front();
        for (City *city : q) {
            if (city->searchData.distance < current->searchData.distance) {
                current = city;
            }
        }
        if (current->searchData.distance != numeric_limits<unsigned>::max()) {
            for (RoadTo &r : current->roads) {
                City* temp;
                for (City &city : planet->cities) {
                    if (city.name == r.city->name) {
                        temp = &city;
                        break;
                    }
                }
                if (temp->searchData.distance > current->searchData.distance + r.length) {
                    temp->searchData.distance = current->searchData.distance + r.length;
                }
            }
        }
        if (result.count(current->name) == 0 && current->searchData.distance != numeric_limits<unsigned>::max()) {
